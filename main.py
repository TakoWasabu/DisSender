...
import threading, time, random, requests, urllib.parse, itertools
...
def configure_thread_pool(max_threads):
    global executor
...
def generate_bypass_string():
    length = dpg.get_value("bypass_count_input")

    # None または 空文字ならデフォルト値 4 を設定
    if not length:
        length = 4
    else:
        try:
            length = int(length)  # 文字列を整数に変換
        except ValueError:
            length = 4  # 無効な値ならデフォルトの 4 を設定
...
# メインウィンドウ
with dpg.window(label="DisRaider - By takowasabu", tag="MainWindow", width=1100, height=400):
    with dpg.group(horizontal=True):
        ...
        with dpg.child_window(tag="main_content", border=False):
            ...
            with dpg.group(tag="ChatSender", show=False):
                ...
                dpg.add_input_text(label="Bypass Count", tag="bypass_count_input", default_value="4")
            ...
            with dpg.group(tag="Settings", show=False):
                ...
                dpg.add_input_int(label="Number of threads", tag="max_thread_input", default_value=10, min_value=1, max_value=50, callback=lambda s, a, u: configure_thread_pool(a))
